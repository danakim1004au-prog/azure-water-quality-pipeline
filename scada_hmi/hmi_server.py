...
def _require(path: Path, how: str) -> Path:
    """Fail with a clear, actionable message if a required input is missing."""
    if not path.exists():
        raise SystemExit(
            f"Required file not found: {path}\n"
            f"It is generated by `{how}`. Generate the demo artifacts first:\n"
...
wells      = pd.read_csv(_require(WELLS_CSV, "python scripts/generate_sample_data.py"))
readings   = pd.read_csv(READINGS_CSV, parse_dates=["reading_date"])
forecasts  = pd.read_csv(_require(FORECASTS_CSV, "python ml/forecast_train.py"),
                         parse_dates=["reading_date", "target_date"])
anom_scores= pd.read_csv(_require(ANOMALY_SC_CSV, "python ml/anomaly_unsupervised.py"),
